...
export class RugCheckService {
  ...
  public async evaluateToken(mint: string, creatorPubkey?: string): Promise<RugCheckResult> {
    ...
    // 1. Dev Deployment Frequency Check (Only for valid pubkeys)
    const now = Date.now();
    if (creatorPubkey && creatorPubkey.trim().length >= 32) {
      ...
      if (devData.count > 3) {
        risks.push(`Dev is spam creating tokens (${devData.count} launches in 10m)`);
        riskScore += 40;
      }
    }
