...
export class WhaleTracker {
  ...
  private seedAlphaWallets(): void {
    // Seed verified profitable alpha wallets (configurable via environment or memory)
  ...
  public evaluateTrade(trade: TradeEvent, tokenSymbol?: string): WhaleAlert | null {
    ...
    const isLargeWhale = sol >= 2.0; // Significant SOL buy on bonding curve

    if (isAlpha || isLargeWhale) {
      const alphaInfo = this.alphaWallets.get(trade.traderPublicKey);
      const label = alphaInfo ? alphaInfo.label : `Whale (${sol.toFixed(2)} SOL)`;
      const conviction = alphaInfo ? Math.min(100, alphaInfo.winRate + Math.floor(sol * 5)) : Math.min(95, Math.floor(sol * 15));
      ...
      console.log(
        chalk.cyan.bold(
          ...
          `  • Mint: ${alert.mint}\n` +
          `  • Amount: ${alert.solAmount.toFixed(3)} SOL\n` +
